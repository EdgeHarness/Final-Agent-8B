...
`run_harness()` — [agent.py:323-470](../harness/agent.py#L323-L470).
...
([agent.py:325](../harness/agent.py#L325)) — and injected under
...
- Steps naming a tool not in `TOOLS` are dropped
  ([agent.py:315](../harness/agent.py#L315)).
...
- The plan *request* is popped from the context
  ([agent.py:344](../harness/agent.py#L344)), so the model never sees
...
[`normalize_date`](../harness/agent.py#L110) resolves against the
...
([tools.py:149](../harness/tools.py#L149)), and the failure message
...
([agent.py:426-461](../harness/agent.py#L426-L461)):
...
([agent.py:473](../harness/agent.py#L473)) re-reads the task against
...
([agent.py:362](../harness/agent.py#L362)), and the repeat-budget
...
Plan, verify, and every repair round are paid out of the same call counter as
ordinary tool calls — the `while llm.calls < cfg.max_calls` condition counts
*model invocations*, not tool executions. The scaffolding does not get free
turns. (`cfg` is the run's `RunConfig`; the module-level `MAX_CALLS` is only its
default.)
